...
<Steps>
	1. Upload a schema. 2. Ensure that your endpoints are added in Endpoint
	Management. 3. Set the schema to `active` if it is not already done. 4. Set
	the Schema Validation zone-wide action from `none` to `log`. 5. Send test
	traffic that violates the schema. 6. View test traffic in Security Events by
	filtering for **Service** > **API Shield - Schema Validation**. 7. Optional: -
	Set a single endpoint to `block`. - Set the Schema Validation zone-wide to
	`block`. - Temporarily override all schemas zone-wide to `none`. - Remove the
	temporary override.
</Steps>
