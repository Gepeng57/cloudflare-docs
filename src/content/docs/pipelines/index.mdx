...
<Feature
	header="HTTP as a source"
	href="/pipelines/build-with-pipelines/sources/http"
>
	Each pipeline generates a globally scalable HTTP endpoint, which supports
	authentication and CORS settings.
</Feature>

<Feature
	header="Workers API"
	href="/pipelines/build-with-pipelines/sources/workers-apis/"
>
	Send data to a pipeline directly from a Cloudflare Worker.
</Feature>

<Feature
	header="Customize output settings"
	href="/pipelines/build-with-pipelines/output-settings"
>
	Define batch sizes and enable compression to generate output files that are
	efficient to query.
</Feature>
